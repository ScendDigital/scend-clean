...
/** Simple scorecard (300Ã¢â‚¬â€œ850) */
...
function rateFromScore(loanType, score, basePrime) {
  ...
  // Credit Card Ã¢â‚¬â€ absolute APR
  if (s >= 780) return 15;
  if (s >= 740) return 18;
  if (s >= 700) return 22;
  if (s >= 660) return 26;
...
export default function LoanTool() {
  ...
  if (!scoreOK) hardFails.push(`Credit score below minimum for ${loanType} (needs Ã¢â€°Â¥ ${minScore}).`);
  if (!dtiOK) hardFails.push(`Post-loan DTI ${Number.isFinite(postDTI) ? pct(postDTI) : "Ã¢â‚¬â€"} exceeds limit ${pct(dtiLimit)}.`);
  if (!ptiOK) hardFails.push(`Payment-to-income ${Number.isFinite(pti) ? pct(pti) : "Ã¢â‚¬â€"} exceeds cap ${pct(ptiLimit)}.`);
  ...
  function principalFromPayment_CreditCard(payPlan, r, targetMonths, minPct, minAbs) {
    ...
    // Min-payment constraint: pay Ã¢â€°Â¥ max(minAbs, minPct * P) -> P Ã¢â€°Â¤ pay/minPct and pay Ã¢â€°Â¥ minAbs.
  ...
  const exportToPDF = () => {
    ...
    const html = `
      <html>
        <head>
          <title>Scend Ã¢â‚¬Â¢ Loan Summary</title>
          <meta name="viewport" content="width=device-width, initial-scale=1" />
          <style>${styles}</style>
        </head>
        <body>
          <h1>Scend Ã¢â‚¬Â¢ Loan Summary</h1>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <h1 className="text-2xl font-semibold">Scend Ã¢â‚¬Â¢ Loan Tool</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <label className="block">
            <div className="text-sm mb-1">
              Credit age (years)
              <span className="block text-xs text-gray-500">Ã¢â€°Ë† time since first account and average age</span>
            </div>
      ...
      <div ref={printRef} className="mt-4 border rounded p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1">
          ...
          {/* DTI/PTI */}
          <div>Pre-loan DTI: <strong>{Number.isFinite(preDTI) ? pct(preDTI) : "Ã¢â‚¬â€"}</strong></div>
          <div>Post-loan DTI: <strong className={dtiOK ? "" : "text-red-600"}>{Number.isFinite(postDTI) ? pct(postDTI) : "Ã¢â‚¬â€"}</strong> <span className="text-sm text-gray-600"> (limit ~{pct(dtiLimit)})</span></div>
          <div>Payment / Income (PTI): <strong className={ptiOK ? "" : "text-red-600"}>{Number.isFinite(pti) ? pct(pti) : "Ã¢â‚¬â€"}</strong> <span className="text-sm text-gray-600"> (cap ~{pct(ptiLimit)})</span></div>
        </div>
        ...
        <div className={`mt-4 border rounded p-3 ${verdictTone}`}>
          ...
          {hardFails.length === 0 && borderline && (
            <div className="mt-2 text-sm">Close! YouÃ¢â‚¬â„¢re within range but slightly tight on score and/or affordability.</div>
          )}
        </div>
        ...
        <div className="mt-3">
          ...
          <ul className="list-disc list-inside space-y-1 text-sm">
            {Number.isFinite(targetMaxPaymentDTI) && (
              <li>To pass DTI for {loanType}, target this loanÃ¢â‚¬â„¢s payment Ã¢â€°Â¤ <strong>{ZAR.format(targetMaxPaymentDTI)}</strong> (DTI limit {pct(dtiLimit)}).</li>
            )}
            {Number.isFinite(targetMaxPaymentPTI) && (
              <li>Keep payment Ã¢â€°Â¤ <strong>{ZAR.format(targetMaxPaymentPTI)}</strong> to meet PTI cap {pct(ptiLimit)}.</li>
            )}
            {!scoreOK && (
              <li>Boost credit score: lower utilisation (aim Ã¢â€°Â¤ 30%), avoid new inquiries for a few months, and resolve any late payments. A higher band often reduces your rate.</li>
            )}
            {loanType === "Vehicle" && cleanNum(balloonPct) > 35 && (
              <li>Consider lowering the balloon (Ã¢â€°Â¤ 35%) or extending term modestly to reduce monthly strain.</li>
            )}
            {loanType === "Credit Card" && (
              <>
                <li>Your first minimum should be around <strong>{ZAR.format(calc.ccMinPayment)}</strong>. Paying only the minimum prolongs payoffÃ¢â‚¬â€aim higher.</li>
        ...
        <div className="mt-5 border rounded p-3 bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1">
            ...
            {loanType !== "Credit Card" && (
              <div>Max purchase price (Ã¢â€°Ë†): <strong>{ZAR.format(qual.purchasePrice)}</strong></div>
            )}
          </div>
