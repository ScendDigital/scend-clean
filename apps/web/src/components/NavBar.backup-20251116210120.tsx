    ...
    <nav className="w-full border-b bg-white/80 backdrop-blur sticky top-0 z-50">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <Link href="/" className="flex items-center gap-2">
          {/* Adjust src/size if your logo file path is different */}
          <Image
            src="/scend-logo.png"
            alt="Scend"
            width={40}
        ...
        <div className="hidden items-center gap-3 md:flex">
            ...
            if (item.label === "Home") {
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`${base} ${isActive(item.href) ? active : idle}`}
                >
                  {item.label}
                </Link>
              );
          ...
          <div className="relative">
            <button
              ...
              onBlur={() => {
                // Tiny delay so clicks inside the menu still register
        ...
        {/* Simple mobile placeholder (you can expand later) */}
