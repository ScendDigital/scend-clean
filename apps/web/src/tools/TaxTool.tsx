...
function annualTaxBeforeRebates(taxable: number) {
  ...
  for (const br of b) {
    if (taxable <= br.up) {
      if (!("over" in br)) {
        return taxable * br.rate;
      } else {
        const over = (br as { over: number }).over;
        return br.base + (taxable - over) * br.rate;
      }
    }
  }
  return 0;
...
export default function TaxTool() {
  ...
  const annualTravelTaxable = useMemo(
    () => travelAllowance * (travelTaxablePct / 100),
    [travelAllowance, travelTaxablePct]
  );
  const annualTaxable = useMemo(
    () => annualIncome + annualTravelTaxable,
    [annualIncome, annualTravelTaxable]
  );
  ...
  const periodRem = useMemo(
    () => monthlyEquivalent * (daysWorked / Math.max(1, periodDays)),
    [monthlyEquivalent, daysWorked, periodDays]
  );
  ...
  const annualTax = useMemo(
    () => annualTaxBeforeRebates(annualisedForPaye),
    [annualisedForPaye]
  );
  const annualRebates = useMemo(() => rebatesByAge(age), [age]);
  const monthlyMedCredits = useMemo(
    () => medCredits(1 + Math.max(0, dependents)),
    [dependents]
  );

  // Convert annual tax back to period tax and subtract monthly medical credits proportionally
  const monthlyTax = useMemo(
    () => Math.max(0, (annualTax - annualRebates) / 12),
    [annualTax, annualRebates]
  );
  const periodTaxBeforeCredits = useMemo(
    () => monthlyTax * (daysWorked / Math.max(1, periodDays)),
    [monthlyTax, daysWorked, periodDays]
  );
  const periodTax = useMemo(
    () =>
      Math.max(
        0,
        periodTaxBeforeCredits -
          monthlyMedCredits * (daysWorked / Math.max(1, periodDays))
      ),
    [periodTaxBeforeCredits, monthlyMedCredits, daysWorked, periodDays]
  );
    ...
    <div className="space-y-6">
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <label className="block">
          <span className="text-sm font-medium">
            Medical Aid Dependents (excl. you)
          </span>
          <input
            type="number"
            value={dependents}
        ...
        <label className="block">
          ...
          <select
            value={travelTaxablePct}
            onChange={(e) => setTravelTaxablePct(+e.target.value)}
            className="mt-1 w-full border rounded-xl p-2"
          >
        ...
        <div className="rounded-2xl border p-4">
          ...
          <div className="text-lg font-semibold">
            R {medCredits(1 + Math.max(0, dependents)).toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border p-4">
          ...
          <div className="text-lg font-semibold">
            R{" "}
            {periodRem.toLocaleString(undefined, {
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="rounded-2xl border p-4">
          ...
          <div className="text-2xl font-semibold">
            R{" "}
            {periodTax.toLocaleString(undefined, {
              maximumFractionDigits: 2,
            })}
          </div>
        </div>
        <div className="rounded-2xl border p-4">
          ...
          <div className="text-lg font-semibold">
            R {annualisedForPaye.toLocaleString()}
          </div>
        </div>
        <div className="rounded-2xl border p-4">
          <div className="text-sm text-gray-600">
            Monthly Med Credits (pro-rated)
          </div>
          <div className="text-lg font-semibold">
            R{" "}
            {(
              medCredits(1 + Math.max(0, dependents)) *
              (daysWorked / Math.max(1, periodDays))
      ...
      <p className="text-xs text-gray-500">
        Note: This is an approximate PAYE method using annualisation and
        pro-rating for mid-month payments.
        For payroll production, apply full SARS rules and fringe benefit
        treatment.
      </p>
    </div>
  );
